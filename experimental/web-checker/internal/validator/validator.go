...
import (
	...
	"sf/usecases/web-checker/fetch-all-contacts-go/internal/emailclean"
	"sf/usecases/web-checker/fetch-all-contacts-go/internal/score"
)
...
func (r Result) withScore() Result {
	b := score.FromStatus(r.Syntax.Status == "passed", r.Domain.Status == "passed", r.MX.Status == "passed", r.SMTP.Status == "passed")
...
func toStep(err error, d time.Duration) Step {
	step := Step{LatencyMS: int(d.Milliseconds()), Status: "passed", Reason: "ok"}
	if err != nil {
		step.Status = "failed"
