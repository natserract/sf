    ...
    return (
        <div className="dashboard">
            <header className="dashboard-header">
                <div>
                    <h1 className="dashboard-title">Email Validation Dashboard</h1>
                    <p className="dashboard-subtitle">Run <code>{runId || "demo"}</code> · Real-time results</p>
                </div>
            </header>
                        ...
                        <ResponsiveContainer width="100%" height={220}>
                            <PieChart>
                                <Pie
                                    ...
                                    labelLine={false}
                                >
                                    {(statusBreakdown.data || []).map((_: any, i: number) => (
                ...
                <div className="chart-card">
                    ...
                    {failureReasons.loading ? <Skeleton height={220} /> : (
                        <div className="failure-list">
                            {(failureReasons.data || []).slice(0, 8).map((f: any, i: number) => (
                                <div key={i} className="failure-item">
                                    ...
                                    <div className="failure-bar-wrap">
                                        <div
                                            ...
                                            style={{
                                                width: `${Math.round((f.count / ((failureReasons.data as any[])[0]?.count || 1)) * 100)}%`,
                                            }}
                                        />
                                    </div>
            ...
            <div className="chart-card full-width">
                ...
                {stagePerf.loading ? <Skeleton height={160} /> : (
                    <table className="perf-table">
                        ...
                        <tbody>
                            {((stagePerf.data as any[]) || []).map((row: any) => (
                                <tr key={row.stage}>
            ...
            {/* Results Table — separate full-width section */}
            <div className="chart-card full-width">
