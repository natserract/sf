...
type SyncService struct {
	client     sforce.SalesforceClient
...
func NewSyncService(client sforce.SalesforceClient, dataExtSvc *DataExtensionService, folderSvc *FolderService, db *postgres.DB, logger *zap.Logger) *SyncService {
	return &SyncService{
		client:     client,
...
func (s *SyncService) SyncFolders(ctx context.Context, metrics *SyncMetrics) error {
	...
	var topLevelFolders []sforce.Folder
	var subfolders []sforce.Folder
	folderMap := make(map[string]sforce.Folder) // Map to track all folders by ID
...
func (s *SyncService) SyncFolder(ctx context.Context, folder sforce.Folder, recursive bool, metrics *SyncMetrics) error {
	// Save the folder
	if err := s.folderSvc.SaveFolder(ctx, folder); err != nil {
		metrics.AddFolderFailure()
